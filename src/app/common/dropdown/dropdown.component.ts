...
export class MultiselectDropdown implements OnInit, OnChanges, DoCheck, OnDestroy, ControlValueAccessor, Validator {
  ...
  isSelected(option: IMultiSelectOption): boolean {
    return this.model && this.commonService.findWithAttr(this.model, this.name, option[this.name])
    //return this.model && this.model.indexOf(option[this.name]) > -1;
  }
  ...
  setSelected(_event: Event, option: IMultiSelectOption) {
    ...
    if (!this.disabledSelection) {
      ...
      const index = this.model.indexOf(option[this.name]);
      ...
      if (index > -1) {
        if ((this.settings.minSelectionLimit === undefined) || (this.numSelected > this.settings.minSelectionLimit)) {
          removeItem(index, option[this.name]);
        }
        const parentIndex = option.parentId && this.model.indexOf(option.parentId);
        if (parentIndex > -1) {
          removeItem(parentIndex, option.parentId);
        } else if (this.parents.indexOf(option[this.name]) > -1) {
          this.options.filter(child => this.model.indexOf(child.id) > -1 && child.parentId === option[this.name])
        ...
        if (!isAtSelectionLimit) {
          ...
          var valueArr = this.model.map(function (item) { return item[_thiss.name]});
          var isDuplicate = valueArr.some(function (item, idx) {
            return valueArr.indexOf(item) != idx
          });
          if (isDuplicate) {
            this.model = this.model.filter(function (obj) {
              return obj[_thiss.name] !== option[_thiss.name];
            });
          }

          // if (option.parentId) {
          //   let children = this.options.filter(child => child.id !== option[this.name] && child.parentId === option.parentId);
          //   if (children.every(child => this.model.indexOf(child.id) > -1)) {
          //     addItem(option.parentId);
          //   }
          // } else if (this.parents.indexOf(option[this.name]) > -1) {
          //   let children = this.options.filter(child => this.model.indexOf(child.id) < 0 && child.parentId === option[this.name]);
  ...
  updateTitle() {
    var masterDataSize = this.masterData ? this.masterData.length : this.options.length
    if (this.numSelected === 0 || this.settings.fixedTitle) {
      this.title = (this.texts) ? this.texts.defaultTitle : '';
    } else if (this.settings.displayAllSelectedText && this.model.length === masterDataSize) {
      ...
      if(this.masterData){
       // console.log(JSON.stringify(this.masterData));
        this.title = this.masterData
        .filter((option: IMultiSelectOption) =>
          this.commonService.findWithAttr(this.model, this.name, option[this.name])
        //this.model.indexOf(option[this.name]) > -1
        )
        .map((option: IMultiSelectOption) => option[this.name])
        .join(', ');
  ...
  addChecks(options) {
    let checkedOptions = options
      .filter(function (option: IMultiSelectOption) {
        if (this.model.indexOf(option[this.name]) === -1) {
          this.onAdded.emit(option[this.name]);
          return true;
        }
        return false;
      }.bind(this)).map((option: IMultiSelectOption) => option[this.name]);
  ...
  uncheckAll() {
    if (!this.disabledSelection) {
      ...
      let unCheckedOptions = (!this.searchFilterApplied() ? this.model
        : this.filteredOptions.map((option: IMultiSelectOption) => option[this.name])
      );
